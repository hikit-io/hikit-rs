...
impl App {
    ...
    pub async fn delete_app(&self, client_id: &str) -> anyhow::Result<()> {
        let _ = match &self.db {
            #[cfg(feature = "mongo")]
            Database::Mongo(db) => delete_app(db, client_id).await?,
        };
        Ok(())
    }
    ...
    pub async fn admin_login(&self, username: &str, password: &str) -> anyhow::Result<()> {
        let admin = match &self.db {
            #[cfg(feature = "mongo")]
            Database::Mongo(db) => fetch_admin(db, username, password).await?,
        };
        Ok(())
    }
}
